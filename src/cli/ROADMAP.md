...
**解决**：每个问题项追加操作路径指引，告诉用户打开哪个文件、改什么字段。

## v0.0.8 — 新手被空骨架的大量报错吓到

**痛点**：刚 init-domain 出来空骨架就被 audit 报几十个问题（本体抽象度、覆盖率等），新手不知道这是正常的。

**解决**：`audit --mode simple` 只检查结构完整性，跳过内容质量检测，问题降一级严重度，末尾引导用户切换到全面模式。`--mode full` 维持严格模式。

## v0.0.9 — 失败提示不一致，extract 输出不像人话

**痛点**：data_home 不存在、sample_home 未设置、JSON 缺失等场景的报错信息不统一，有的崩溃、有的静默。extract 输出技术细节（领域名、分数），业务专家看不懂。

**解决**：统一所有失败路径为业务语言提示。extract 摘要改为一句话 + `--verbose` 控制详情。
