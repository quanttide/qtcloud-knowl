...
**解决**：设计 prompt 管线 + 结构化输出解析器 + 编排调用，完成本体发现 → 实例映射 → 关系发现的全自动语义抽取。经 review CLI 审核确认后落库。配合 audit 做质量门禁，实现"文档入库 → 质检 → 发布"闭环。LLM 连接层由 `quanttide-agent` 提供；不强制复用现有 ReActAgent，可按需覆盖或重写。
